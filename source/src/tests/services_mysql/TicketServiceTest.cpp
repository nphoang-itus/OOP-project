...
protected:
    ...
    void SetUp() override
    {
        ...
        ticketRepository = std::make_shared<TicketRepository>(db, passengerRepository, flightRepository, logger);
        service = std::make_shared<TicketService>(ticketRepository, logger);

        // // Setup Passenger
        // auto passengerResult = Passenger::create("John Doe", "john.doe123@abc.com|0123456789|123 Main St", "VN:1264167");
        // ASSERT_TRUE(passengerResult.has_value());
        // auto createPassengerResult = passengerRepository->create(passengerResult.value());
        // ASSERT_TRUE(createPassengerResult.has_value());
        // passenger = std::make_shared<Passenger>(createPassengerResult.value());

        // // Setup Aircraft
        // auto aircraftResult = Aircraft::create("VN123", "A320", "E:100,B:20,F:10");
        // ASSERT_TRUE(aircraftResult.has_value());
        // auto createAircraftResult = aircraftRepository->create(aircraftResult.value());
        // ASSERT_TRUE(createAircraftResult.has_value());
        // auto aircraft = std::make_shared<Aircraft>(createAircraftResult.value());

        // // Setup Flight
        // auto flightNumberResult = FlightNumber::create("VN137");
        ...
        // auto createFlightResult = flightRepository->create(flightResult.value());
        // ASSERT_TRUE(createFlightResult.has_value());
        // flight = std::make_shared<Flight>(createFlightResult.value());

        auto passengerResult = passengerRepository->findById(1).value();
        passenger = std::make_shared<Passenger>(passengerResult);

        auto flightResult = flightRepository->findById(1).value();
        flight = std::make_shared<Flight>(flightResult);
    ...
    {
        // Clean up test data
        auto result = db->execute("DELETE FROM ticket WHERE ticket_number = 'AB12-20230525-0001'");
...
TEST_F(TicketServiceTest, DeleteByIdSuccess)
{
    ...
    auto findResult = service->getById(createResult->getId());
