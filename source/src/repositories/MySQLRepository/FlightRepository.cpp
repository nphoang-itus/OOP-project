...
Result<Flight> FlightRepository::findById(const int &id)
{
    try
    {
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for finding flight by id: " + std::to_string(id));
        ...
        if (!setParamResult)
        {
            ...
            if (_logger)
                _logger->error("Failed to set parameter for finding flight by id: " + std::to_string(id));
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for finding flight by id: " + std::to_string(id));
        ...
        {
            if (_logger)
                _logger->warning("Flight not found with id: " + std::to_string(id));
        ...
        {
            if (_logger)
                _logger->error("Failed to get flight data for id: " + std::to_string(id));
        ...
        if (economySeatsResult.value() > 0)
            seatLayoutStr << "E:" << economySeatsResult.value();
        if (businessSeatsResult.value() > 0)
        {
            if (!seatLayoutStr.str().empty())
                seatLayoutStr << ",";
        ...
        if (_logger)
            _logger->debug("Successfully found flight with id: " + std::to_string(id));
        return Success(flight);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error finding flight by id: " + std::string(e.what()));
...
Result<std::vector<Flight>> FlightRepository::findAll()
{
    try
    {
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for finding all flights");
        ...
        while (dbResult->next().value())
        {
            auto idResult = dbResult->getInt(ColumnName[ID]);
            if (!idResult)
                break; // Break if we can't get the ID, indicating end of results
            ...
            {
                if (_logger)
                    _logger->warning("Skipping invalid flight data");
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Invalid serial number: " + serialNumberResult.value());
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Invalid seat layout: " + seatLayoutStr.str());
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Failed to create aircraft");
                continue;
            }
            aircraft->setId(aircraftIdResult.value());
            ...
            {
                if (_logger)
                    _logger->warning("Invalid flight number: " + flightNumberResult.value());
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Invalid route: " + routeStr.str());
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Invalid schedule: " + scheduleStr.str());
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Failed to create flight");
                continue;
            }
        ...
        if (_logger)
            _logger->debug("Successfully found " + std::to_string(flights.size()) + " flights");
        return Success(flights);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error finding all flights: " + std::string(e.what()));
...
Result<bool> FlightRepository::exists(const int &id)
{
    ...
    {
        if (_logger)
            _logger->debug("Checking existence of flight with id: " + std::to_string(id));

        auto prepareResult = _connection->prepareStatement(EXISTS_QUERY);
        if (!prepareResult)
        {
            if (_logger)
                _logger->error("Failed to prepare statement for checking flight existence");
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for checking flight existence");
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for checking flight existence");
        ...
        {
            if (_logger)
                _logger->debug("Flight not found with id: " + std::to_string(id));
        ...
        {
            if (_logger)
                _logger->error("Failed to get count for flight existence check");
            return Failure<bool>(CoreError("Failed to get count", "COUNT_FAILED"));
        }

        bool exists = countResult.value() > 0;
        if (_logger)
            _logger->debug("Flight " + std::to_string(id) + " " + (exists ? "exists" : "does not exist"));
        return Success(exists);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error checking flight existence: " + std::string(e.what()));
...
Result<size_t> FlightRepository::count()
{
    ...
    {
        if (_logger)
            _logger->debug("Counting total flights");

        auto result = _connection->executeQuery(COUNT_QUERY);
        if (!result)
        {
            if (_logger)
                _logger->error("Failed to execute query for counting flights");
        ...
        {
            if (_logger)
                _logger->debug("No flights found");
            return Success(0);
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to get count");
            return Failure<size_t>(CoreError("Failed to get count", "COUNT_FAILED"));
        }

        size_t count = static_cast<size_t>(countResult.value());
        if (_logger)
            _logger->debug("Total flight count: " + std::to_string(count));
        return Success(count);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error counting flights: " + std::string(e.what()));
...
Result<Flight> FlightRepository::create(const Flight &flight)
{
    ...
    {
        if (_logger)
            _logger->debug("Creating new flight");
        ...
        {
            if (_logger)
                _logger->error("Failed to check flight existence");
            return Failure<Flight>(CoreError("Failed to check flight existence", "DB_ERROR"));
        }
        if (existsResult.value())
        {
            if (_logger)
                _logger->error("Failed to create because flight : " + flight.getFlightNumber().toString() + " exists");
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for creating flight");
            return Failure<Flight>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
            ...
            !setDepartureTimeResult || !setArrivalTimeResult || !setStatusResult)
        {
            ...
            if (_logger)
                _logger->error("Failed to set parameters for creating flight");
        ...
        if (!result)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to execute statement for creating flight");
        ...
        if (!idResult)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to get last insert id for flight");
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for creating seat availability");
            return Failure<Flight>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        ...
        for (const auto &[classCode, count] : seatLayout.getSeatCounts())
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                if (!setFlightIdResult || !setSeatNumberResult)
                {
                    ...
                    if (_logger)
                        _logger->error("Failed to set parameters for creating seat availability");
                    return Failure<Flight>(CoreError("Failed to set parameters", "PARAM_FAILED"));
                }
                ...
                if (!seatResult)
                {
                    _connection->freeStatement(seatStmtId);
                    if (_logger)
                        _logger->error("Failed to create seat availability record");
        ...
        if (_logger)
            _logger->debug("Successfully created flight with id: " + std::to_string(idResult.value()));
    ...
    catch (const std::exception &e)
    {
        _connection->rollbackTransaction();
        if (_logger)
            _logger->error("Error creating flight: " + std::string(e.what()));
        return Failure<Flight>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
Result<Flight> FlightRepository::update(const Flight &flight)
{
    try
    {
        ...
        {
            if (_logger)
                _logger->error("Flight not found with id: " + std::to_string(flight.getId()));
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for updating flight");
            return Failure<Flight>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
            ...
            !setDepartureTimeResult || !setArrivalTimeResult || !setStatusResult || !setIdResult)
        {
            ...
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to set parameters for updating flight");
            return Failure<Flight>(CoreError("Failed to set parameters", "PARAM_FAILED"));
        }
        ...
        if (!result)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to execute statement for updating flight");
            return Failure<Flight>(CoreError("Failed to execute statement", "EXECUTE_FAILED"));
        }
        ...
        if (_logger)
            _logger->debug("Successfully updated flight with id: " + std::to_string(flight.getId()));
        return Success(flight);
    }
    catch (const std::exception &e)
    {
        _connection->rollbackTransaction();
        if (_logger)
            _logger->error("Error updating flight: " + std::string(e.what()));
        return Failure<Flight>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
Result<bool> FlightRepository::deleteById(const int &id)
{
    ...
    {
        if (_logger)
            _logger->debug("Deleting flight with id: " + std::to_string(id));
        ...
        {
            if (_logger)
                _logger->error("Flight not found with id: " + std::to_string(id));
        ...
        // First delete seat availability records to avoid foreign key constraint
        std::string deleteSeatQuery = "DELETE FROM flight_seat_availability WHERE flight_id = ?";
        auto seatPrepareResult = _connection->prepareStatement(deleteSeatQuery);
        if (!seatPrepareResult)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to prepare statement for deleting seat availability");
            return Failure<bool>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int seatStmtId = seatPrepareResult.value();

        auto setSeatParamResult = _connection->setInt(seatStmtId, 1, id);
        if (!setSeatParamResult)
        {
            _connection->freeStatement(seatStmtId);
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to set parameter for deleting seat availability");
            return Failure<bool>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        if (!seatResult)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to execute statement for deleting seat availability");
            return Failure<bool>(CoreError("Failed to execute statement", "EXECUTE_FAILED"));
        }

        if (_logger)
            _logger->debug("Deleted seat availability records for flight id: " + std::to_string(id));
        ...
        if (!prepareResult)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to prepare statement for deleting flight");
            return Failure<bool>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
        ...
        if (!setParamResult)
        {
            ...
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to set parameter for deleting flight");
            return Failure<bool>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        if (!result)
        {
            _connection->rollbackTransaction();
            if (_logger)
                _logger->error("Failed to execute statement for deleting flight");
            return Failure<bool>(CoreError("Failed to execute statement", "EXECUTE_FAILED"));
        }
        ...
        if (_logger)
            _logger->debug("Successfully deleted flight with id: " + std::to_string(id));
        return Success(true);
    }
    catch (const std::exception &e)
    {
        _connection->rollbackTransaction();
        if (_logger)
            _logger->error("Error deleting flight: " + std::string(e.what()));
        return Failure<bool>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
Result<Flight> FlightRepository::findByFlightNumber(const FlightNumber &number)
{
    ...
    {
        if (_logger)
            _logger->debug("Finding flight by flight number: " + number.toString());

        auto prepareResult = _connection->prepareStatement(FIND_BY_NUMBER_QUERY);
        if (!prepareResult)
        {
            if (_logger)
                _logger->error("Failed to prepare statement for finding flight by flight number: " + number.toString());
            return Failure<Flight>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for finding flight by flight number: " + number.toString());
            return Failure<Flight>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for finding flight by flight number: " + number.toString());
            return Failure<Flight>(CoreError("Failed to execute query", "QUERY_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->warning("Flight not found with flight number: " + number.toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to get flight data for flight number: " + number.toString());
            return Failure<Flight>(CoreError("Failed to get flight data", "DATA_ERROR"));
        }
        ...
        if (_logger)
            _logger->debug("Successfully found flight with flight number: " + number.toString());
        return Success(flight);
    }
    catch (const std::exception &e)
    {
...
Result<bool> FlightRepository::existsFlight(const FlightNumber &number)
{
    ...
    {
        if (_logger)
            _logger->debug("Checking existence of flight with flight number: " + number.toString());
        ...
        {
            if (_logger)
                _logger->debug("Flight not found with flight number: " + number.toString());
            return Success(false);
        }
        ...
        bool exists = countResult.value() > 0;
        if (_logger)
            _logger->debug("Flight " + number.value() + " " + (exists ? "exists" : "does not exist"));
        return Success(exists);
    }
    catch (const std::exception &e)
    {
...
Result<std::vector<Flight>> FlightRepository::findFlightByAircraft(const AircraftSerial &serial)
{
    ...
    {
        if (!_logger)
            _logger->debug("Finding aircraft exists");

        auto prepareResult = _connection->prepareStatement(FIND_FLIGHT_BY_SERIAL);
        if (!prepareResult)
        {
            if (_logger)
                _logger->error("Failed to prepare statement for finding aircraft by serial: " + serial.toString());
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for finding flight by serial: " + serial.toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for finding flight by serial: " + serial.toString());
            return Failure<std::vector<Flight>>(CoreError("Failed to execute query", "QUERY_FAILED"));
        }
        ...
        if (_logger)
            _logger->debug("Successfully found " + std::to_string(flights.size()) + " aircraft");
        return Success(flights);
    }
    catch (const std::exception &e)
    {
...
std::map<SeatNumber, bool> FlightRepository::getSeatAvailability(const Flight &flight) const
{
    ...
    try
    {
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for getting seat availability");
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for getting seat availability");
            return seatAvailability;
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for getting seat availability");
            return seatAvailability;
        }
        ...
        while (dbResult->next().value())
        {
            ...
            {
                if (_logger)
                    _logger->warning("Invalid seat availability data");
                continue;
            }
            ...
            {
                if (_logger)
                    _logger->warning("Invalid seat number: " + seatNumberResult.value());
                continue;
            }
    ...
    {
        if (_logger)
            _logger->error("Error getting seat availability: " + std::string(e.what()));
    }
    return seatAvailability;
}
...
Result<bool> FlightRepository::reserveSeat(const Flight &flight, const SeatNumber &seatNumber)
{
    ...
    {
        if (_logger)
            _logger->debug("Reserving seat " + seatNumber.toString() + " for flight " + flight.getFlightNumber().toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for checking seat availability");
            return Failure<bool>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        ...
        if (!setFlightIdResult || !setSeatNumberResult)
        {
            ...
            if (_logger)
                _logger->error("Failed to set parameters for checking seat availability");
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for checking seat availability");
            return Failure<bool>(CoreError("Failed to execute query", "QUERY_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to get seat availability count");
        ...
        {
            if (_logger)
                _logger->error("Failed to get count result");
        ...
        {
            if (_logger)
                _logger->error("Seat is not available");
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for reserving seat");
            return Failure<bool>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        ...
        if (!setFlightIdResult || !setSeatNumberResult)
        {
            ...
            if (_logger)
                _logger->error("Failed to set parameters for reserving seat");
            return Failure<bool>(CoreError("Failed to set parameters", "PARAM_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to execute update for reserving seat");
        ...
        {
            if (_logger)
                _logger->error("Failed to reserve seat - no rows affected");
            return Failure<bool>(CoreError("Failed to reserve seat", "RESERVE_FAILED"));
        }

        if (_logger)
            _logger->debug("Seat reservation successful");
        return Success(true);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error reserving seat: " + std::string(e.what()));
        return Failure<bool>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
Result<bool> FlightRepository::releaseSeat(const Flight &flight, const SeatNumber &seatNumber)
{
    ...
    {
        if (_logger)
            _logger->debug("Releasing seat " + seatNumber.toString() + " for flight " + flight.getFlightNumber().toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for releasing seat");
            return Failure<bool>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set flight_id parameter");
            return Failure<bool>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set seat_number parameter");
            return Failure<bool>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to execute update for releasing seat");
            return Failure<bool>(CoreError("Failed to execute update", "UPDATE_FAILED"));
        }

        bool success = result.value() > 0;
        if (_logger)
            _logger->debug("Seat release " + std::string(success ? "successful" : "failed"));
        return Success(success);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error releasing seat: " + std::string(e.what()));
        return Failure<bool>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
Result<std::vector<SeatNumber>> FlightRepository::getAvailableSeats(const Flight &flight)
{
    ...
    {
        if (_logger)
            _logger->debug("Getting available seats for flight " + flight.getFlightNumber().toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for getting available seats");
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for getting available seats");
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for getting available seats");
        ...
        while (dbResult->next().value())
        {
            ...
            {
                if (_logger)
                    _logger->warning("Invalid seat number data");
                continue;
            }
        ...
        if (_logger)
            _logger->debug("Found " + std::to_string(availableSeats.size()) + " available seats");
        return Success(availableSeats);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error getting available seats: " + std::string(e.what()));
...
Result<std::vector<SeatNumber>> FlightRepository::getReservedSeats(const Flight &flight)
{
    ...
    {
        if (_logger)
            _logger->debug("Getting reserved seats for flight " + flight.getFlightNumber().toString());
        ...
        {
            if (_logger)
                _logger->error("Failed to prepare statement for getting reserved seats");
            return Failure<std::vector<SeatNumber>>(CoreError("Failed to prepare statement", "PREPARE_FAILED"));
        }
        int stmtId = prepareResult.value();
        ...
        if (!setParamResult)
        {
            _connection->freeStatement(stmtId);
            if (_logger)
                _logger->error("Failed to set parameter for getting reserved seats");
            return Failure<std::vector<SeatNumber>>(CoreError("Failed to set parameter", "PARAM_FAILED"));
        }
        ...
        {
            if (_logger)
                _logger->error("Failed to execute query for getting reserved seats");
            return Failure<std::vector<SeatNumber>>(CoreError("Failed to execute query", "QUERY_FAILED"));
        }
        ...
        if (_logger)
            _logger->debug("Found " + std::to_string(reservedSeats.size()) + " reserved seats");
        return Success(reservedSeats);
    }
    catch (const std::exception &e)
    {
        if (_logger)
            _logger->error("Error getting reserved seats: " + std::string(e.what()));
        return Failure<std::vector<SeatNumber>>(CoreError("Database error: " + std::string(e.what()), "DB_ERROR"));
    }
}
...
{
    if (_logger)
        _logger->debug("Checking if seat is available for flight: " + flight.getFlightNumber().toString() + ", seat: " + seatNumber.toString());
