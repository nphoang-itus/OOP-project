...
{
    if (_logger)
        _logger->debug("Getting passenger with passport: " + passport.toString());
...
{
    if (_logger)
        _logger->debug("Getting all passengers");
...
{
    if (_logger)
        _logger->debug("Checking if passenger exists with passport: " + passport.toString());
...
{
    if (_logger)
        _logger->debug("Creating passenger with passport: " + passenger.getPassport().toString());
    ...
    {
        if (_logger)
            _logger->error("Failed to check passenger existence");
        return Failure<Passenger>(CoreError("Failed to check passenger existence", "DB_ERROR"));
    }
    if (existsResult.value())
    {
        if (_logger)
            _logger->error("Passenger with passport " + passenger.getPassport().toString() + " already exists");
...
{
    if (_logger)
        _logger->debug("Updating passenger with passport: " + passenger.getPassport().toString());

    // Check if passenger exists by ID (since passport might be changing)
    auto existsResult = _passengerRepository->exists(passenger.getId());
    if (!existsResult)
    {
        if (_logger)
            _logger->error("Failed to check passenger existence");
    ...
    {
        if (_logger)
            _logger->error("Passenger with ID " + std::to_string(passenger.getId()) + " not found");
...
{
    if (_logger)
        _logger->debug("Deleting passenger with passport: " + passport.toString());
    ...
    {
        if (_logger)
            _logger->error("Passenger with passport " + passport.toString() + " not found");
    ...
    {
        if (_logger)
            _logger->error("Failed to get passenger details");
    ...
    {
        if (_logger)
            _logger->error("Failed to check if passenger can be deleted");
        return Failure<bool>(canDeleteResult.error());
    }
    if (!canDeleteResult.value())
    {
        if (_logger)
            _logger->error("Cannot delete passenger with active bookings");
...
{
    if (_logger)
        _logger->debug("Checking if passenger can be deleted: " + passport.toString());
    ...
    {
        if (_logger)
            _logger->error("Failed to get passenger");
        return Failure<bool>(passengerResult.error());
    }
    ...
    {
        if (_logger)
            _logger->error("Failed to check passenger bookings");
...
{
    if (_logger)
        _logger->debug("Checking if passenger has active bookings: " + passport.toString());
    ...
    {
        if (_logger)
            _logger->error("Failed to get passenger tickets");
...
{
    if (_logger)
        _logger->debug("Getting total flight count for passenger: " + passport.toString());
