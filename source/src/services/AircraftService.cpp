...
{
    if (_logger)
        _logger->debug("Getting aircraft with serial: " + serial.toString());
...
{
    if (_logger)
        _logger->debug("Getting all aircraft");
...
{
    if (_logger)
        _logger->debug("Checking if aircraft exists with serial: " + serial.toString());
...
{
    if (_logger)
        _logger->debug("Creating aircraft with serial: " + aircraft.getSerial().toString());
    ...
    {
        if (_logger)
            _logger->error("Failed to check aircraft existence");
        return Failure<Aircraft>(CoreError("Failed to check aircraft existence", "DB_ERROR"));
    }
    if (existsResult.value())
    {
        if (_logger)
            _logger->error("Aircraft with serial " + aircraft.getSerial().toString() + " already exists");
...
{
    if (_logger)
        _logger->debug("Updating aircraft with serial: " + aircraft.getSerial().toString());
    ...
    {
        if (_logger)
            _logger->error("Aircraft with serial " + aircraft.getSerial().toString() + " not found");
...
{
    if (_logger)
        _logger->debug("Deleting aircraft with serial: " + serial.toString());
    ...
    {
        if (_logger)
            _logger->error("Aircraft with serial " + serial.toString() + " not found");
    ...
    {
        if (_logger)
            _logger->error("Failed to get aircraft details");
    ...
    {
        if (_logger)
            _logger->error("Failed to check aircraft flights");
        return Failure<bool>(flightsResult.error());
    }
    if (!flightsResult.value().empty())
    {
        if (_logger)
            _logger->error("Cannot delete aircraft that is used in flights");
    ...
    {
        if (_logger)
            _logger->error("Failed to check aircraft tickets");
        return Failure<bool>(ticketsResult.error());
    }
    if (!ticketsResult.value().empty())
    {
        if (_logger)
            _logger->error("Cannot delete aircraft that has tickets");
...
{
    if (_logger)
        _logger->debug("Getting available seat classes for aircraft: " + serial.toString());
    ...
    {
        if (_logger)
            _logger->error("Failed to get aircraft");
    ...
    if (seatLayout.hasSeatClass("E"))
        seatClasses.push_back("ECONOMY");
    if (seatLayout.hasSeatClass("B"))
        seatClasses.push_back("BUSINESS");
    if (seatLayout.hasSeatClass("F"))
        seatClasses.push_back("FIRST");
...
{
    if (_logger)
        _logger->debug("Getting available seats for aircraft: " + serial.toString() + ", class: " + seatClass);
    ...
    {
        if (_logger)
            _logger->error("Invalid seat class: " + seatClass);
    ...
    {
        if (_logger)
            _logger->error("Aircraft does not have seat class: " + seatClass);
    ...
    {
        if (_logger)
            _logger->error("Failed to get tickets");
...
{
    if (_logger)
        _logger->debug("Checking if seat is available for aircraft: " + serial.toString() + ", seat: " + seatNumber);
    ...
    {
        if (_logger)
            _logger->error("Invalid seat number: " + seatNumber);
